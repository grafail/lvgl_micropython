...
esp_err_t esp_lcd_new_panel_io_3wire_spi(const esp_lcd_panel_io_3wire_spi_config_t *io_config, esp_lcd_panel_io_handle_t *ret_io)
{
    ...
    ESP_GOTO_ON_ERROR(gpio_set_level(panel_io->cs_io_num, cs_idle_level), err, TAG, "Set CS level failed");
    ESP_GOTO_ON_ERROR(gpio_set_level(panel_io->scl_io_num, sda_scl_idle_level), err, TAG, "Set SCL level failed");
...
static esp_err_t panel_io_del(esp_lcd_panel_io_t *io)
{
    ...
    if (!panel_io->flags.del_keep_cs_inactive) {
        ESP_RETURN_ON_ERROR(gpio_reset_pin(panel_io->cs_io_num), TAG, "Reset CS line failed");
    } else {
        ESP_LOGW(TAG, "Delete but keep CS line inactive");
    }
    ESP_RETURN_ON_ERROR(gpio_reset_pin(panel_io->scl_io_num), TAG, "Reset SCL line failed");
    ESP_RETURN_ON_ERROR(gpio_reset_pin(panel_io->sda_io_num), TAG, "Reset SDA line failed");
    free(panel_io);
...
static esp_err_t spi_write_byte(esp_lcd_panel_io_3wire_spi_t *panel_io, int dc_bit, uint8_t data)
{
    ...
    for (uint8_t i = 0; i < data_bits; i++) {
        // Send DC bit first
        if (data_bits == 9 && i == 0) {
            ESP_RETURN_ON_ERROR(gpio_set_level(panel_io->sda_io_num, dc_bit), TAG, "Set SDA level failed");
        } else { // Then send data bit
            // SDA set to data bit
            ESP_RETURN_ON_ERROR(gpio_set_level(panel_io->sda_io_num, data_temp & write_order_mask), TAG, "Set SDA level failed");
            // Get next bit
            data_temp = (write_order_mask == WRITE_ORDER_LSB_MASK) ? data_temp >> 1 : data_temp << 1;
        }
        // Generate SCL active edge

        ESP_RETURN_ON_ERROR(gpio_set_level(panel_io->scl_io_num, scl_active_befor_level), TAG, "Set SCL level failed");
        delay_us(scl_half_period_us);
        ESP_RETURN_ON_ERROR(gpio_set_level(panel_io->scl_io_num, scl_active_after_level), TAG, "Set SCL level failed");
        delay_us(scl_half_period_us);
    }
...
static esp_err_t spi_write_package(esp_lcd_panel_io_3wire_spi_t *panel_io, bool is_cmd, uint32_t data)
{
    ...
    // CS active
    ESP_RETURN_ON_ERROR(gpio_set_level(panel_io->cs_io_num, !cs_idle_level), TAG, "Set CS level failed");
    delay_us(time_us);
    ...
    ESP_RETURN_ON_ERROR(gpio_set_level(panel_io->scl_io_num, sda_scl_idle_level), TAG, "Set SCL level failed");
    ESP_RETURN_ON_ERROR(gpio_set_level(panel_io->sda_io_num, sda_scl_idle_level), TAG, "Set SDA level failed");
    delay_us(time_us);
    // CS inactive
    ESP_RETURN_ON_ERROR(gpio_set_level(panel_io->cs_io_num, cs_idle_level), TAG, "Set CS level failed");
    delay_us(time_us);
